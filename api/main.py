...
def alerts() -> dict:
    """
    Evalúa las métricas acumuladas y genera alertas operativas básicas.
    Esta mejora permite interpretar las métricas, no solo mostrarlas.
    """

    datos = resumen_metricas()
    alertas = []
    estado_operativo = "normal"

    solicitudes = datos["solicitudes_totales"]
    errores_validacion = datos["errores_validacion"]
    anomalias = datos["solicitudes_con_anomalias"]
    latencia_promedio = datos["latencia_promedio_ms"]

    if solicitudes > 0:
        tasa_errores = errores_validacion / solicitudes
        tasa_anomalias = anomalias / solicitudes
    else:
        tasa_errores = 0
        tasa_anomalias = 0

    if tasa_errores >= 0.30:
        alertas.append(
            "Alta proporción de errores de validación. Revisar calidad de datos enviados a /predict."
        )
        estado_operativo = "advertencia"

    if tasa_anomalias >= 0.30:
        alertas.append(
            "Alta proporción de datos fuera del rango histórico. Posible señal inicial de drift."
        )
        estado_operativo = "advertencia"

    if latencia_promedio > 1000:
        alertas.append(
            "Latencia promedio elevada. Revisar recursos del contenedor o carga de solicitudes."
        )
        estado_operativo = "critico"

    if not alertas:
        alertas.append("No se detectan alertas operativas relevantes.")

    return {
        "estado_operativo": estado_operativo,
        "alertas": alertas,
        "tasa_errores_validacion": round(tasa_errores, 4),
        "tasa_anomalias": round(tasa_anomalias, 4),
        "latencia_promedio_ms": latencia_promedio,
        "accion_recomendada": (
            "Revisar logs, validar datos de entrada y evaluar drift si las alertas persisten."
            if estado_operativo != "normal"
            else "Continuar monitoreo periódico."
        ),
    }